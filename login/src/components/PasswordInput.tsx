        ...
        <div className="space-y-2">
            ...
            <div className="relative">
                <input
                    ...
                    maxLength={72}
                    className="w-full bg-midnight-100/80 border border-slate-700/50 rounded-xl pl-4 pr-12 py-4 text-slate-50 placeholder-slate-500 focus:outline-none focus:border-indigo-500/60 focus:ring-1 focus:ring-indigo-500/30 transition-all duration-200"
                />
                <button
                    type="button"
            ...
            {showStrengthMeter && value.length > 0 && (
                <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span
                            className={`text-xs font-medium ${strength === 'weak' ? 'text-red-400' : strength === 'medium' ? 'text-yellow-400' : 'text-emerald-400'
                                }`}
                        >
                    ...
                    <div className="space-y-1">
                        {requirements.map((req, index) => (
                            <div
                                key={index}
                                className={`flex items-center gap-2 text-xs transition-colors ${req.met ? 'text-emerald-400' : 'text-slate-500'
                                    }`}
                            >
                                {req.met ? (
